<template>
  ...
  <v-btn @click="start_simulation" rounded="0" :class="{ hidden: playIsHidden }">
    ...
    <v-btn @click="stop_simulation" rounded="0" :class="{ hidden: stopIsHidden }">
    <svg-icon
      type="mdi"
      :path="pathStop"
      class=""
...
async function start_stop_animation(event) {
  if (simulation_running == false) {
    ...
    const count_wins = await LottoExperiment(n_slider.value, k_slider.value, count_trials.value);
    ...
    alert(
      "Etwas ist schiefgelaufen. Die Webseite wird nun automatisch neu geladen...",
    );
    location.reload();
  }
}
