...
var import_serialize = require("#util/serialize.js");
...
function renderValue(value, formatter, squash) {
  ...
  if (typeof value === "object" && Symbol.iterator in value && !(value instanceof String)) {
    ...
    }).filter((e) => e.length > 0).join(squash ? "" : " ");
  }
  ...
  if (typeof value === "object") {
    ...
    return formatter.text((0, import_serialize.serialize)(value) ?? "undefined");
  }
...
function renderDictionary(value, formatter) {
  ...
  for (const [k, v] of entries) {
    ...
    const suppressKey = (0, import_Type.isObject)(v) && import_Diagnostic.Diagnostic.presentationOf(v) === "flag";
    if (!suppressKey) {
...
function valueFor(value) {
  ...
  if (import_Diagnostic.Diagnostic.value in value) {
    const proxied = import_Diagnostic.Diagnostic.valueOf(value);
    if (proxied === value) {
...
function presentationFor(value) {
  ...
  if (import_Diagnostic.Diagnostic.presentation in value) {
    return import_Diagnostic.Diagnostic.presentationOf(value);
  }
  const proxied = import_Diagnostic.Diagnostic.valueOf(value);
...
function renderDiagnostic(value, formatter, ignorePresentation) {
  const presentation = ignorePresentation ? void 0 : presentationFor(value);
  const logValue = valueFor(value);
  if (logValue === value) {
    ignorePresentation = true;
  } else {
    ignorePresentation = void 0;
    value = logValue;
  }
  switch (presentation) {
    case void 0:
