...
((Diagnostic2) => {
  ...
  function errorMessage(details) {
    ...
    function format({ id, message: message2, path, cause }) {
      ...
      if (hasCause) {
        ...
        format((0, import_Error.asError)(cause));
      } else {
  ...
  function elapsed() {
    return {
      startedAt: import_Time.Time.nowUs,
      get time() {
        return (0, import_TimeUnit.Millis)(import_Time.Time.nowUs - this.startedAt);
      },
      toString() {
        return import_Duration.Duration.format(this.time);
      },
