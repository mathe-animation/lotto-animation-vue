...
const DEFAULT_MDNS_TTL = (0, import_TimeUnit.Seconds)(120);
const MAX_MDNS_MESSAGE_SIZE = 1232;
const PtrRecord = (name, ptr, ttl = DEFAULT_MDNS_TTL, flushCache = false) => ({
  name,
  value: ptr,
  ttl,
  recordType: 12 /* PTR */,
...
const ARecord = (name, ip, ttl = DEFAULT_MDNS_TTL, flushCache = false) => ({
  name,
  value: ip,
  ttl,
  recordType: 1 /* A */,
...
const AAAARecord = (name, ip, ttl = DEFAULT_MDNS_TTL, flushCache = false) => ({
  name,
  value: ip,
  ttl,
  recordType: 28 /* AAAA */,
...
const TxtRecord = (name, entries, ttl = DEFAULT_MDNS_TTL, flushCache = false) => ({
  name,
  value: entries,
  ttl,
  recordType: 16 /* TXT */,
...
const SrvRecord = (name, srv, ttl = DEFAULT_MDNS_TTL, flushCache = false) => ({
  name,
  value: srv,
  ttl,
  recordType: 33 /* SRV */,
...
class DnsCodec {
  static decode(message) {
    try {
      const reader = new import_DataReader.DataReader(message);
  ...
  static decodeRecord(reader, message) {
    ...
    const recordClass = classInt & 32767;
    const ttl = (0, import_TimeUnit.Seconds)(reader.readUInt32());
  ...
  static decodeQName(reader, message, visited = /* @__PURE__ */ new Set()) {
    ...
    const messageReader = new import_DataReader.DataReader(message);
  ...
  static decodeRecordValue(valueBytes, recordType, message) {
    switch (recordType) {
      case 12 /* PTR */:
        return this.decodeQName(new import_DataReader.DataReader(valueBytes), message);
      case 33 /* SRV */:
  ...
  static decodeAaaaRecord(valueBytes) {
    const reader = new import_DataReader.DataReader(valueBytes);
    return (0, import_Ip.ipv6BytesToString)(reader.readByteArray(16));
  }
  static decodeARecord(valueBytes) {
    const reader = new import_DataReader.DataReader(valueBytes);
    return (0, import_Ip.ipv4BytesToString)(reader.readByteArray(4));
  }
  static encode({
    messageType,
    transactionId = 0,
  ...
  static encodeRecord(record) {
    ...
    writer.writeUInt32(import_TimeUnit.Seconds.of(ttl));
  ...
  static encodeARecord(ip) {
    ...
    return (0, import_Ip.ipv4ToBytes)(ip);
  }
  static encodeAaaaRecord(ip) {
    ...
    return (0, import_Ip.ipv6ToBytes)(ip);
  }
