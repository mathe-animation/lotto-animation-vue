...
class RuntimeService {
  ...
  add(worker) {
    ...
    if (typeof worker === "function") {
      try {
        ...
        this.#crash((0, import_Error.asError)(e));
      }
      return;
    }
  ...
  get [import_Diagnostic.Diagnostic.value]() {
    return import_Diagnostic.Diagnostic.node("\u{1F6E0}", `workers for ${this.#env.name}`, {
  ...
  #cancelWorker(worker) {
    ...
    const cancel = () => {
      ...
      try {
        if (worker.close) {
          return Promise.resolve(worker.close()).catch(unhandled).finally(() => this.delete(worker));
        }
        if (worker[Symbol.asyncDispose]) {
          return Promise.resolve(worker[Symbol.asyncDispose]?.()).catch(unhandled).finally(() => this.delete(worker));
        }
