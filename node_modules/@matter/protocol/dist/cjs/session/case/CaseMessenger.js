...
class CaseClientMessenger extends import_SecureChannelMessenger.SecureChannelMessenger {
  sendSigma1(sigma1, options) {
    return this.send(sigma1, import_types.SecureMessageType.Sigma1, import_CaseMessages.TlvCaseSigma1, options);
  }
  async readSigma2() {
    const {
      payload,
    ...
    switch (messageType) {
      ...
      default:
        throw new import_general.MatterFlowError(
          `Received unexpected message type while expecting CASE Sigma2(Resume): ${messageType}, expected: ${import_types.SecureMessageType.Sigma2} or ${import_types.SecureMessageType.Sigma2Resume}`
        );
    }
  }
  sendSigma3(sigma3) {
