    ...
    return this.send(
      ...
      import_types.TlvStatusResponse.encode({ status, interactionModelRevision: import_model.Specification.INTERACTION_MODEL_REVISION }),
      {
        ...
        logContext: {
          ...
          status: `${import_types.Status[status] ?? "unknown"}(${import_general.Diagnostic.hex(status)})`,
          ...options?.logContext
        }
      }
    );
  ...
  throwIfErrorStatusMessage(message, logHint) {
    ...
    if (status !== import_types.Status.Success)
      throw new import_types.ReceivedStatusResponseError(
        `Received error status: ${import_types.Status[status]}(${status})${logHint ? ` (${logHint})` : ""}`,
        status
      );
  }
  get session() {
...
class InteractionServerMessenger extends InteractionMessenger {
  async handleRequest(recipient) {
    ...
    try {
      while (continueExchange) {
        ...
        switch (message.payloadHeader.messageType) {
          ...
          case 10 /* TimedRequest */: {
            ...
            await this.sendStatus(import_types.Status.Success, {
      ...
      let errorStatusCode = import_types.Status.Failure;
  ...
  async sendDataReportMessage(dataReport, waitForAck = true) {
    ...
    if (encodedMessage.byteLength > this.exchange.maxPayloadSize) {
      throw new import_general.MatterFlowError(
        `DataReport with ${encodedMessage.byteLength}bytes is too long to fit in a single chunk (${this.exchange.maxPayloadSize}bytes), This should not happen! Data: ${import_general.Diagnostic.json(
          dataReportToSend
        )}`
      );
    }
    const logContext = {
      ...import_Subscription.Subscription.diagnosticOf(dataReportToSend),
    ...
    if (dataReportToSend.suppressResponse) {
      try {
        ...
        import_errors.UnexpectedMessageError.accept(e);
      ...
      await this.waitForSuccess("DataReport", { timeout: waitForAck ? void 0 : (0, import_general.Millis)(500) });
    }
  }
  /**
    ...
    switch (report.kind) {
      ...
      case "event-value": {
        ...
        const data = {
          ...
          hasFabricSensitiveData: true
          // There are no Fabric sensitive events as of now. If ever added sanitizing needs to be added
        };
        return data;
      }
      case "event-status": {
   ...
   */
  async sendWriteResponse(response, options) {
    await this.send(7 /* WriteResponse */, import_types.TlvWriteResponse.encode(response), {
      logContext: options?.logContext ? { for: options.logContext } : void 0
    });
  }
  /**
   * Wait for and decode the next WriteRequest message (for chunked writes).
   */
  async readNextWriteRequest() {
    const message = await this.nextMessage(6 /* WriteRequest */, void 0, "WriteRequest-chunk");
    return {
      writeRequest: import_types.TlvWriteRequest.decode(message.payload),
      message
    };
  }
  /**
   ...
   */
  async sendInvokeResponseChunk(response) {
    await this.send(
      9 /* InvokeResponse */,
   ...
   */
  async sendInvokeResponse(response) {
    await this.send(9 /* InvokeResponse */, import_types.TlvInvokeResponseForSend.encode(response));
  }
}
class IncomingInteractionClientMessenger extends InteractionMessenger {
  async waitFor(expectedMessageInfo, messageType, timeout) {
  ...
  async *readDataReports() {
    while (true) {
      ...
      if (report.moreChunkedMessages) {
        ...
        this.sendStatus(import_types.Status.Success, {
          multipleMessageInteraction: true,
          logContext: this.#logContextOf(report)
  ...
  #logContextOf(report) {
    return {
      subId: report.subscriptionId === void 0 ? void 0 : import_Subscription.Subscription.idStrOf(report.subscriptionId),
...
class InteractionClientMessenger extends IncomingInteractionClientMessenger {
  ...
  async send(messageType, payload, options) {
    ...
    try {
      if (this.exchange.channel.closed) {
        throw new import_errors.SessionClosedError("The exchange channel is closed. Please connect the device first.");
      }
      return await this.exchange.send(messageType, payload, options);
    } catch (error) {
      if (this.#exchangeProvider.supportsReconnect && (error instanceof import_errors.RetransmissionLimitReachedError || error instanceof import_errors.SessionClosedError) && !options?.multipleMessageInteraction) {
        logger.debug(
          `${error instanceof import_errors.RetransmissionLimitReachedError ? "Retransmission limit reached" : "Channel not connected"}, trying to reconnect and resend the message.`
        );
        await this.exchange.close();
   ...
   */
  async sendInvokeCommand(invokeRequest, expectedProcessingTime) {
    ...
    await this.send(8 /* InvokeRequest */, import_types.TlvInvokeRequest.encode(invokeRequest), {
      expectAckOnly: false,
      expectedProcessingTime
    });
    const allInvokeResponses = [];
    let finalResponse;
    while (true) {
      ...
      const response = import_types.TlvInvokeResponse.decode(responseMessage.payload);
  ...
  async requestWithSuppressedResponse(requestMessageType, requestSchema, request, expectedProcessingTime) {
    try {
  ...
  async request(requestMessageType, requestSchema, responseMessageType, responseSchema, request, expectedProcessingTime) {
    await this.send(requestMessageType, requestSchema.encode(request), {
      expectAckOnly: false,
      expectedProcessingTime
    });
