"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var ChipPAAuthorities_exports = {};
__export(ChipPAAuthorities_exports, {
  TestCert_PAA_FFF1_Cert: () => TestCert_PAA_FFF1_Cert,
  TestCert_PAA_FFF1_PrivateKey: () => TestCert_PAA_FFF1_PrivateKey,
  TestCert_PAA_FFF1_PublicKey: () => TestCert_PAA_FFF1_PublicKey,
  TestCert_PAA_FFF1_SKID: () => TestCert_PAA_FFF1_SKID,
  TestCert_PAA_NoVID_Cert: () => TestCert_PAA_NoVID_Cert,
  TestCert_PAA_NoVID_PrivateKey: () => TestCert_PAA_NoVID_PrivateKey,
  TestCert_PAA_NoVID_PublicKey: () => TestCert_PAA_NoVID_PublicKey,
  TestCert_PAA_NoVID_SKID: () => TestCert_PAA_NoVID_SKID,
  TestCert_WithoutAuthKeyId: () => TestCert_WithoutAuthKeyId
});
module.exports = __toCommonJS(ChipPAAuthorities_exports);
var import_general = require("#general");
/**
 * @license
 * Copyright 2022-2026 Matter.js Authors
 * SPDX-License-Identifier: Apache-2.0
 */
const TestCert_PAA_NoVID_PublicKey = import_general.Bytes.fromHex(
  "0410ef02a81a87b68121fba8d31978f807a317e50aa8a828446828914b933de8edd4a5c39c9ff71a4ce3647fd7f62653b7d2495fcba4c0f47f876880039e07204a"
);
const TestCert_PAA_NoVID_PrivateKey = import_general.Bytes.fromHex(
  "e1f073c934853baffb38bf7e8bdad7a0a674107c7769892a0ff2e06c1a2ef7a7"
);
const TestCert_PAA_NoVID_SKID = import_general.Bytes.fromHex("785CE705B86B8F4E6FC793AA60CB43EA696882D5");
const TestCert_PAA_NoVID_Cert = import_general.Bytes.fromHex(
  ""
);
const TestCert_PAA_FFF1_PrivateKey = import_general.Bytes.fromHex(
  "6512caecaecfc543d60623161597162f014684c565a129b62fd28c27ab1ccc50"
);
const TestCert_PAA_FFF1_PublicKey = import_general.Bytes.fromHex(
  "04b6cb6372887f2928f5bac81aa9d93ae2431cada9d79e242f65177ef9ced932a28ecd03baaf6a8fca184a1a503542960d453f303f1f19421d751e8f8f1a9a9b75"
);
const TestCert_PAA_FFF1_SKID = import_general.Bytes.fromHex("6AFD22771F511FECBF1641976710DCDC31A1717E");
const TestCert_PAA_FFF1_Cert = import_general.Bytes.fromHex(
  ""
);
const TestCert_WithoutAuthKeyId = import_general.Bytes.fromBase64(
  ""
);
//# sourceMappingURL=ChipPAAuthorities.js.map
