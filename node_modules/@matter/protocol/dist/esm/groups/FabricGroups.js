 ...
 */
import { Bytes, hex, InternalError, MatterFlowError } from "#general";
import { KeySets } from "#groups/KeySets.js";
import { MessagingState } from "#groups/MessagingState.js";
import { MATTER_EPOCH_OFFSET_US } from "#types";
import { Groups } from "./Groups.js";
const GROUP_SECURITY_INFO = Bytes.fromString("GroupKey v1.0");
const IPK_DEFAULT_EPOCH_START_TIME = MATTER_EPOCH_OFFSET_US;
class FabricGroups {
  #fabric;
  #groups;
  #messagingState;
  ...
  constructor(fabric, storage) {
    ...
    this.#keySets.add({
      ...
      epochStartTime0: IPK_DEFAULT_EPOCH_START_TIME,
      // with the Year 2000-offset that's basically 0 and always ok, but only for the IPK key
  ...
  */
  ...
  async setFromGroupKeySet(groupKeySet) {
    ...
    const operationalEpochKey1 = epochKey1 !== null ? await this.#fabric.crypto.createHkdfKey(epochKey1, globalId, GROUP_SECURITY_INFO) : null;
    const operationalEpochKey2 = epochKey2 !== null ? await this.#fabric.crypto.createHkdfKey(epochKey2, globalId, GROUP_SECURITY_INFO) : null;
    this.#keySets.add({
      ...groupKeySet,
