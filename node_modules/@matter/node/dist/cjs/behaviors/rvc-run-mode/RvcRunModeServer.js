...
var RvcRunModeServer_exports = {};
__export(RvcRunModeServer_exports, {
  RvcRunModeServer: () => RvcRunModeServer
});
module.exports = __toCommonJS(RvcRunModeServer_exports);
var import_mode_base = require("#behaviors/mode-base");
var import_mode_base2 = require("#clusters/mode-base");
var import_rvc_run_mode = require("#clusters/rvc-run-mode");
var import_general = require("#general");
var import_RvcRunModeBehavior = require("./RvcRunModeBehavior.js");
/**
 * @license
 * Copyright 2022-2026 Matter.js Authors
 * SPDX-License-Identifier: Apache-2.0
 */
class RvcRunModeServer extends import_RvcRunModeBehavior.RvcRunModeBehavior {
  initialize() {
  ...
  #assertSupportedModes() {
    ...
    if (!this.state.supportedModes.some(
      ({ modeTags }) => modeTags.some(({ value }) => value === import_rvc_run_mode.RvcRunMode.ModeTag.Idle)
    )) {
      throw new import_general.ImplementationError("Provided supportedModes need to include at least one Idle mode tag");
    }
    if (!this.state.supportedModes.some(
      ({ modeTags }) => modeTags.some(({ value }) => value === import_rvc_run_mode.RvcRunMode.ModeTag.Cleaning)
    )) {
      throw new import_general.ImplementationError("Provided supportedModes need to include at least one Cleaning mode tag");
    }
    if (this.state.supportedModes.some(({ modeTags }) => {
      let exclusiveModeCounter = 0;
      exclusiveModeCounter += modeTags.some(({ value }) => value === import_rvc_run_mode.RvcRunMode.ModeTag.Idle) ? 1 : 0;
      exclusiveModeCounter += modeTags.some(({ value }) => value === import_rvc_run_mode.RvcRunMode.ModeTag.Cleaning) ? 1 : 0;
      exclusiveModeCounter += modeTags.some(({ value }) => value === import_rvc_run_mode.RvcRunMode.ModeTag.Mapping) ? 1 : 0;
...
//# sourceMappingURL=RvcRunModeServer.js.map
