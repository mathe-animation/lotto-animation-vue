...
class GeneralCommissioningServer extends import_GeneralCommissioningBehavior.GeneralCommissioningBehavior {
  ...
  async #armFailSafe({ breadcrumb, expiryLengthSeconds }, session) {
    import_protocol.NodeSession.assert(session, "Failsafe may only be armed on a secure session");
    ...
    try {
      if (!commissioner.isFailsafeArmed && this.agent.get(import_administrator_commissioning.AdministratorCommissioningServer).state.windowStatus !== import_administrator_commissioning2.AdministratorCommissioning.CommissioningWindowStatus.WindowNotOpen && !session.isPase) {
        throw new import_general.MatterFlowError("Cannot arm failsafe using CASE while commissioning window is opened");
      }
      if (commissioner.isFailsafeArmed) {
        await commissioner.failsafeContext.extend(
          session.fabric,
          (0, import_general.Seconds)(expiryLengthSeconds),
          this.context.exchange
        );
      } else {
        if (expiryLengthSeconds === 0) return SuccessResponse;
        const failsafe = new import_ServerNodeFailsafeContext.ServerNodeFailsafeContext(this.endpoint, {
  ...
  async commissioningComplete() {
    ...
    if (import_protocol.NodeSession.is(session) && session.isPase || import_protocol.GroupSession.is(session)) {
      return {
    ...
    import_protocol.SecureSession.assert(session, "Commissioning may only complete on a secure session");
    ...
    logger.info(
      ...
      import_general.Diagnostic.dict({
        fabric: `${import_general.hex.fixed(fabric.globalId, 16)} (#${fabric.fabricIndex})`,
        node: import_general.hex.fixed(fabric.nodeId, 16)
      })
    );
    return SuccessResponse;
  }
