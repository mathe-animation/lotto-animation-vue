...
var ServiceAreaServer_exports = {};
__export(ServiceAreaServer_exports, {
  ServiceAreaBaseServer: () => ServiceAreaBaseServer,
  ServiceAreaServer: () => ServiceAreaServer
});
module.exports = __toCommonJS(ServiceAreaServer_exports);
var import_service_area = require("#clusters/service-area");
var import_general = require("#general");
var import_types = require("#types");
var import_ServiceAreaBehavior = require("./ServiceAreaBehavior.js");
/**
 * @license
 * Copyright 2022-2026 Matter.js Authors
 * SPDX-License-Identifier: Apache-2.0
 */
const ServiceAreaBase = import_ServiceAreaBehavior.ServiceAreaBehavior.with(import_service_area.ServiceArea.Feature.Maps, import_service_area.ServiceArea.Feature.ProgressReporting);
class ServiceAreaBaseServer extends ServiceAreaBase {
  ...
  #assertSelectedAreas(areas) {
    ...
    for (const areaId of areas) {
      if (!this.state.supportedAreas.some((area) => area.areaId === areaId)) {
        throw new import_types.ValidationError(`AreaID ${areaId} is not in the supported areas list`);
      }
  ...
  #assertProgress(progress) {
    ...
    for (const area of progress) {
      if (areaIds.has(area.areaId)) {
        throw new import_types.ValidationError(`AreaID ${area.areaId} is not unique`);
      }
      areaIds.add(area.areaId);
      if (!this.state.supportedAreas.some((supportedArea) => supportedArea.areaId === area.areaId)) {
        throw new import_types.ValidationError(`AreaID ${area.areaId} is not in the supported areas list`);
      }
    }
  }
  /**
  ...
  selectAreas({ newAreas }) {
    ...
    if (result.status === import_service_area.ServiceArea.SelectAreasStatus.Success && validatedAreas !== void 0) {
      ...
      if (this.state.progress !== void 0 && validatedAreas.length > 0) {
        ...
        for (const area of validatedAreas) {
          progress.push({
            areaId: area,
            status: import_service_area.ServiceArea.OperationalStatus.Pending
          });
        }
  ...
  assertSelectServiceArea({ newAreas }) {
    ...
    for (const areaId of newAreas) {
      if (!this.state.supportedAreas.some((area) => area.areaId === areaId)) {
        return {
          result: {
            status: import_service_area.ServiceArea.SelectAreasStatus.UnsupportedArea,
    ...
    return {
      result: {
        status: import_service_area.ServiceArea.SelectAreasStatus.Success,
        statusText: ""
      },
      validatedAreas
    };
  ...
  assertSkipServiceArea({ skippedArea }) {
    if (this.state.selectedAreas.length === 0) {
      return {
        status: import_service_area.ServiceArea.SkipAreaStatus.InvalidAreaList,
        statusText: ""
      };
    }
    if (!this.state.selectedAreas.some((area) => area === skippedArea)) {
      return {
        status: import_service_area.ServiceArea.SkipAreaStatus.InvalidSkippedArea,
        statusText: `AreaID ${skippedArea} is not in the selected areas list`
      };
    }
    return {
      status: import_service_area.ServiceArea.SkipAreaStatus.Success,
      statusText: ""
    };
  }
  removeSupportedAreasEntry(areaId) {
    ...
    this.state.selectedAreas = (0, import_general.deepCopy)(this.state.selectedAreas.filter((area) => area !== areaId));
    if (this.state.currentArea === areaId) {
      this.state.currentArea = null;
    }
    if (this.state.progress !== void 0) {
      this.state.progress = (0, import_general.deepCopy)(this.state.progress.filter((area) => area.areaId !== areaId));
    }
    this.state.supportedAreas = (0, import_general.deepCopy)(supportedAreas.filter((area) => area.areaId !== areaId));
  }
  /**
  ...
  removeSupportedMapsEntry(mapId) {
    ...
    if (mapToRemove !== void 0) {
      ...
      this.state.supportedMaps = (0, import_general.deepCopy)(supportedMaps.filter((map) => map.mapId !== mapToRemove.mapId));
    }
  }
}
