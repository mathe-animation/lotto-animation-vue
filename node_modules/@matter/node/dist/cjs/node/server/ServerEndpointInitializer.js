...
class ServerEndpointInitializer extends import_EndpointInitializer.EndpointInitializer {
  ...
  initializeDescendant(endpoint) {
    ...
    if (!(import_DescriptorServer.DescriptorServer.id in endpoint.behaviors.supported)) {
      endpoint.behaviors.inject(import_DescriptorServer.DescriptorServer, void 0, false);
    }
  }
  ...
  createBacking(endpoint, type) {
    ...
    return new import_ServerBehaviorBacking.ServerBehaviorBacking(endpoint, type, store, endpoint.behaviors.optionsFor(type));
  }
  /**
  ...
  behaviorsInitialized(agent) {
    if (agent.env.has(import_protocol.FabricManager)) {
      const fabricIndices = agent.env.get(import_protocol.FabricManager).fabrics.map((fabric) => fabric.fabricIndex);
      if (fabricIndices.length > 0) {
        return (0, import_FabricScopedDataHandler.limitEndpointAttributeDataToAllowedFabrics)(agent.endpoint, fabricIndices);
      }
    }
  }
}
