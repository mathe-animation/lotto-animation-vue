 ...
 */
const logger = import_general.Logger.get("Transition");
class Transitions {
  #endpoint;
  #timer;
  #config;
  ...
  start(transition) {
    ...
    if (!this.#config.manageTransitions || !changePerS || !Number.isFinite(changePerS)) {
      ...
      logger.info(this.#logPrefix, "Set", import_general.Diagnostic.strong(name), "to", import_general.Diagnostic.strong(targetValue));
    ...
    if (cyclic && targetValue !== void 0) {
      if (min === void 0 || max === void 0) {
        throw new import_general.ImplementationError("Cyclic transition requires min and max values");
      }
    ...
    const attr = {
      ...
      prevStepAt: (0, import_general.Timestamp)(import_general.Time.nowMs - (this.#config.stepInterval ?? Transitions.DEFAULT_STEP_INTERVAL)),
      distanceLeft
    };
    ...
    logger.info(
      this.#logPrefix,
      "Transitioning",
      import_general.Diagnostic.strong(name),
  ...
  #updateRemainingTime(newRemainingTime, fromCommand = false) {
    ...
    this.#config.remainingTimeEvent?.emit(newRemainingTime, previousRemainingTime, void 0);
  }
  /**
  ...
  transitionImmediately(owner, name, targetValue) {
    const result = this.applyUpdates(owner, { [name]: targetValue });
    return import_general.MaybePromise.then(result, () => {
    });
  }
  /**
  ...
  transitionGradually(_attr) {
    if (this.#timer === void 0) {
      this.#timer = import_general.Time.getPeriodicTimer(
  ...
  finish(name) {
    const finishOne = (state) => {
      ...
      logger.debug(this.#logPrefix, "Transition of", import_general.Diagnostic.strong(name2), "finished");
  ...
  get remainingTime() {
    if (this.#config.manageTransitions === false) {
      if (this.#config.transitionEndTimeMs !== void 0) {
        const remaining = (0, import_general.Timespan)(import_general.Time.nowMs, this.#config.transitionEndTimeMs).duration;
        if (remaining < 0) {
          return 0;
        }
  ...
  async step(behavior) {
    ...
    for (const prop of this) {
      ...
      let nextValue = cyclic ? (0, import_general.addValueWithOverflow)(currentValue, changeSinceLastStep, min, max) : min !== void 0 && max !== void 0 ? (0, import_general.cropValueRange)(currentValue + changeSinceLastStep, min, max) : currentValue + changeSinceLastStep;
    ...
    if (import_general.MaybePromise.is(applyPromise)) {
      await applyPromise;
    }
  }
  ...
  #stepError(name, message) {
    logger.warn(this.#logPrefix, "Not transitioning", import_general.Diagnostic.strong(name), "because", message);
    this.stop(name);
  }
  ...
  get #logPrefix() {
    return import_general.Diagnostic.squash(import_general.Diagnostic.strong(`${this.#endpoint}#${this.#config.type.name}`), `:`);
  }
  #externalTimeOf(ms) {
    return Math.round(ms / (this.#config.externalTimeUnit ?? Transitions.DEFAULT_EXTERNAL_TIME_UNIT));
  }
  #internalTimeOf(externalUnits) {
    return (0, import_general.Millis)(externalUnits * (this.#config.externalTimeUnit ?? Transitions.DEFAULT_EXTERNAL_TIME_UNIT));
  }
  /**
  ...
  #instrumentProperty(name) {
    ...
    if (!this.#observers) {
      this.#observers = new import_general.ObserverGroup();
    }
...
((Transitions2) => {
  Transitions2.DEFAULT_STEP_INTERVAL = (0, import_general.Millis)(100);
  Transitions2.DEFAULT_EXTERNAL_TIME_UNIT = (0, import_general.Millis)(100);
})(Transitions || (Transitions = {}));
