...
class SoftwareUpdateManager extends import_Behavior.Behavior {
  ...
  async initialize() {
    ...
    this.internal.otaService = this.internal.services.get(import_protocol.DclOtaUpdateService);
  ...
  async #checkProductForUpdates(infos, includeStoredUpdates) {
    ...
    logger.info(
      ...
      import_general.Diagnostic.dict({
        ...
        softwareVersion,
        file: `ota/${fd.text}`,
        peers: [...otaEndpoints.values()].map(({ endpoint }) => import_Node.Node.forEndpoint(endpoint).id)
      })
    );
