  ...
  return {
    ...
    changedEventFor(key) {
      ...
      if (!Number.isFinite(id)) {
        ...
        if (field !== void 0) {
          event = events[`${(0, import_general.camelize)(field.name)}$Changed`];
        }
      }
...
function createReference(resource, internals, session) {
  ...
  const reference = {
    ...
    set value(_value) {
      throw new import_general.InternalError(`Cannot set root reference for ${internals.supervisor.schema.name}`);
    },
    get expired() {
      return expired;
    },
    ...
    change(mutator) {
      ...
      if (values === internals.values) {
        ...
        const properties = values[import_protocol.Val.properties] ? values[import_protocol.Val.properties](this.rootOwner, session) : void 0;
    ...
    refresh() {
      throw new import_general.InternalError(`Cannot refresh root reference for ${internals.supervisor.schema.name}`);
    }
  };
  ...
  function startWrite() {
    ...
    transaction.beginSync();
    if ((0, import_protocol.hasRemoteActor)(session) && !session.interactionStarted && session.interactionComplete && !session.interactionComplete.isObservedBy(internals.interactionObserver)) {
  ...
  function computePreCommitChange(name) {
    ...
    precommitValues[name] = (0, import_general.deepCopy)(newValue);
