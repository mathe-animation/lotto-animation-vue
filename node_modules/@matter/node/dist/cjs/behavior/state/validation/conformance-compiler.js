...
function astToFunction(schema, supervisor) {
  ...
  switch (compiledNode.code) {
    ...
    case import_conformance_util.Code.Value:
      validator = compiledNode.value === void 0 ? disallowValue : isNullable ? void 0 : requireValue;
      break;
  ...
  function compile(ast2) {
    switch (ast2?.type) {
      ...
      case import_model.Conformance.Special.Otherwise:
  ...
  function createGroup(param) {
    ...
    if (param[param.length - 1]?.type === import_model.Conformance.Flag.Deprecated) {
  ...
  function disallowEnumValue(schema2) {
    return (location) => {
      throw new import_protocol2.EnumValueConformanceError(
        schema2,
        location,
  ...
  function addEnumMemberValidation(mainValidator) {
    ...
    for (const member of members) {
      ...
      let ast2 = member.conformance.ast;
      if (ast2.type === import_model.Conformance.Special.Choice) {
        ast2 = ast2.param.expr;
      }
      const compiledNode2 = compile(ast2);
      let memberValidator;
    ...
    return (value, session, location) => {
      ...
      if (typeof value === "number") {
        if (!memberValidators[value]) {
          throw new import_protocol2.UnknownEnumValueError(
            location,
