...
var import_cluster_behavior_utils = require("#behavior/cluster/cluster-behavior-utils.js");
var import_Migration = require("#behavior/state/migrations/Migration.js");
...
class BehaviorBacking {
  ...
  constructor(endpoint, type, store, options) {
    ...
    this.#type = type;
    this.#protocol = endpoint.env.get(import_ProtocolService.ProtocolService);
    this.#changeTracking = endpoint.env.get(import_ChangeNotificationService.ChangeNotificationService);
    this.store = store;
  ...
  [import_general.Construction.construct](agent) {
    ...
    try {
      ...
      return import_general.MaybePromise.then(promise, () => this.#protocol.addCluster(this), crash);
    } catch (e) {
      crash(e);
    }
  }
  ...
  initializeDataSource() {
    ...
    if (this.store.initialValues !== void 0) {
      import_Migration.Migration.migrate(this.type, this.store.initialValues);
    }
  ...
  createBehavior(agent, type) {
    ...
    if (behavior instanceof type || (0, import_cluster_behavior_utils.isClientBehavior)(type)) {
      return behavior;
    }
  ...
  get datasource() {
    if (!this.#datasource) {
      throw new import_general.InternalError("Datasource not yet initialized");
    }
  ...
  get datasourceOptions() {
    return {
      entropy: this.#endpoint.env.get(import_general.Entropy),
      location: {
