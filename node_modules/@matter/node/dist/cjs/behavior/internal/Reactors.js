...
var Reactors_exports = {};
__export(Reactors_exports, {
  Reactors: () => Reactors
});
module.exports = __toCommonJS(Reactors_exports);
 ...
 */
const logger = import_general.Logger.get("Reactors");
class Reactors {
  #backing;
  ...
  remove(selector) {
    ...
    return import_general.MatterAggregateError.allSettled(
...
class ReactorBacking {
  ...
  constructor(reactors, observable, reactor, { offline, once, lock }) {
    ...
    const reactorListener = ((...args) => {
      ...
      try {
        if (!this.#trampoline) {
          ...
          if (!this.#nodeActivity) {
            this.#nodeActivity = this.#endpoint.env.get(import_NodeActivity.NodeActivity);
          }
      ...
      if (!alreadyReacting) {
        if (import_general.MaybePromise.is(resolution)) {
      ...
      } else if (import_general.MaybePromise.is(resolution)) {
   ...
   */
  #react(args) {
    const originalContext = import_Contextual.Contextual.contextOf(args[args.length - 1]);
    let context;
    if (!this.#offline) {
    ...
    if (context) {
      ...
      if (import_general.MaybePromise.is(result) && !this.#observable.isAsync) {
    ...
    const command = (0, import_protocol.hasRemoteActor)(originalContext) && originalContext.command;
    try {
      const reactor = (context2) => {
        return this.#reactWithContext(context2, this.#owner.backing, args);
      };
      let result = import_LocalActorContext.LocalActorContext.act(this.toString(), reactor, {
        command,
    ...
    return new Promise((resolve, reject) => {
      const deferredReaction = async () => {
        try {
          ...
          reject((0, import_general.asError)(e));
        }
      };
  ...
  async #lockThenReact(context, backing, args) {
    if (!this.#lock) {
      throw new import_general.InternalError("No locks to acquire");
    }
    ...
    for (const resource of context.transaction.resources) {
      if (resource.lockedBy !== context.transaction) {
        throw new import_general.InternalError(`Lock of ${resource} should be held by ${this} but is not`);
      }
    }
    const reactor = this.#bindReactor(context, backing);
   ...
   */
  #augmentError(cause) {
    const error = (0, import_general.asError)(cause);
