...
import { NodeId, StatusResponse } from "#types";
...
class OtaSoftwareUpdateRequestorServer extends OtaSoftwareUpdateRequestorBehavior {
  ...
  async #online() {
    ...
    if (this.state.activeOtaProviders.length > 0) {
      this.#scheduleUpdateQuery(
        this.state.updateInProgressDetails === null ? Seconds(Math.floor(Math.random() * 599) + 120) : void 0
      );
    }
  ...
  #scheduleInitialQuery() {
    this.#scheduleUpdateQuery(Seconds(30), 4 /* Commissioned */);
  }
  async #handlePreviousUpdateOnStart({ newSoftwareVersion, location, updateToken }) {
    ...
    try {
      ...
      await ep.commandsOf(OtaSoftwareUpdateProviderClient).notifyUpdateApplied({ softwareVersion: newSoftwareVersion, updateToken });
      logger.info(
  ...
  /** Enables or disables the update queries based on the new value of updatePossible */
    ...
    if (newValue) {
      if (!this.internal.updateQueryTimer?.isRunning || this.state.updateState === OtaSoftwareUpdateRequestor.UpdateState.Idle) {
        const seconds = Seconds(Math.floor(Math.random() * 599) + 60);
  ...
  #handleUpdatedDefaultOtaProviders(newValues) {
    ...
    if (this.internal.updateQueryTimer === void 0 && this.state.activeOtaProviders.length > 0) {
      this.#scheduleUpdateQuery(Seconds(Math.floor(Math.random() * 599) + 60), 3 /* Announced */);
    }
  }
  /**
  ...
  async announceOtaProvider({
    ...
    await Node.forEndpoint(this.endpoint).peers.forAddress(peerAddress);
    if (announcementReason !== OtaSoftwareUpdateRequestor.AnnouncementReason.SimpleAnnouncement) {
      const delay = Seconds(Math.floor(Math.random() * 599) + 1);
  ...
  #scheduleUpdateQuery(delay, reason = 0 /* NewQuery */, provider) {
    ...
    if (delay === void 0 && this.state.updateState !== OtaSoftwareUpdateRequestor.UpdateState.Idle) {
      logger.info(
  ...
  async #updateAvailableFromProvider(query, ep, providerLocation) {
    ...
    switch (status) {
      ...
      case OtaSoftwareUpdateProvider.Status.NotAvailable:
        if (status === OtaSoftwareUpdateProvider.Status.Busy && this.internal.providerRetryCount <= MAX_BUSY_RETRIES) {
          this.#updateState(
  ...
  async #validateApplyUpdate(applyRequest, ep, fileDesignator) {
    ...
    try {
      ({ action, delayedActionTime: applyDelayedActionTime } = await ep.commandsOf(OtaSoftwareUpdateProviderClient).applyUpdateRequest(applyRequest));
    } catch (error) {
    ...
    while (true) {
      ...
      try {
        ...
        if (++downloadTryCount >= 3) {
          const code = error instanceof BdxError ? error.code : void 0;
  ...
  async #handleDownload(endpoint, uri, newSoftwareVersion) {
    if (uri.startsWith("bdx://")) {
      if (!this.state.transferProtocolsSupported.includes(
        ...
        throw new InternalError("BDX Synchronous not supported");
      }
      this.#updateState(
  ...
  #updateState(newState, reason, targetSoftwareVersion) {
    ...
    if (newState !== OtaSoftwareUpdateRequestor.UpdateState.Downloading && newState !== OtaSoftwareUpdateRequestor.UpdateState.Applying && newState !== OtaSoftwareUpdateRequestor.UpdateState.RollingBack) {
    ...
    if (newState === OtaSoftwareUpdateRequestor.UpdateState.Idle || newState === OtaSoftwareUpdateRequestor.UpdateState.Unknown) {
      this.#scheduleUpdateQuery();
      this.state.updateStateProgress = null;
  ...
  #basicInformationState() {
    const rootEndpoint = Node.forEndpoint(this.endpoint);
    if (!rootEndpoint.behaviors.has(BasicInformationBehavior)) {
      throw new InternalError("BasicInformationBehavior missing");
    }
