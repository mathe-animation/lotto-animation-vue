...
class EnergyEvseModeServer extends EnergyEvseModeBehavior {
  initialize() {
  ...
  #assertSupportedModes() {
    ...
    if (!this.state.supportedModes.some(
      ({ modeTags }) => modeTags.some(({ value }) => value === EnergyEvseMode.ModeTag.Manual) && !modeTags.some(
        ({ value }) => value === EnergyEvseMode.ModeTag.TimeOfUse || value === EnergyEvseMode.ModeTag.SolarCharging
      )
    )) {
