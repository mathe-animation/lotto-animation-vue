 ...
 */
const logger = import_general.Logger.get("ModelVariantTraversal");
class ModelVariantTraversal {
  ...
  clusterState;
  visiting = false;
  modelTraversal = new import_ModelTraversal.ModelTraversal();
  /**
   * Initiate traversal.  The class is stateful so this call should not be invoked while traversal is ongoing.
   */
  traverse(variants) {
    if (this.visiting) {
      throw new import_general.InternalError("ModelVariantVisitor.visit called with active visit; reentrancy not supported");
    }
  ...
  enterCluster(variants) {
    if (variants.tag === import_common.ElementTag.Cluster) {
      this.clusterState = {
  ...
  createVariantDetail(map) {
    ...
    if (!tag) {
      throw new import_general.InternalError("No tag identified in variant set");
    }
    if (!name) {
      throw new import_general.InternalError("No name identified in variant set");
    }
  ...
  const traversal = new class extends ModelVariantTraversal {
    visit(variants2, recurse) {
      ...
      for (let priority = 0; priority < sourceNames.length; priority++) {
        ...
        if (!base || base.parent?.tag !== import_common.ElementTag.Cluster) {
          continue;
        }
        if (!mapEntry) {
          mapEntry = {
