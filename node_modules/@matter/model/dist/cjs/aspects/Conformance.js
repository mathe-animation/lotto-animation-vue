...
var import_parser = require("../parser/index.js");
...
class Conformance extends import_Aspect.Aspect {
  ...
  validateComputation(errorTarget, features) {
    try {
      this.applicabilityFor({ definedFeatures: features ?? /* @__PURE__ */ new Set(), supportedFeatures: /* @__PURE__ */ new Set() });
    } catch (e) {
      errorTarget.error("CANNOT_COMPUTE_CONFORMANCE", `Error computing conformance: ${(0, import_general.asError)(e).message}`);
    }
  }
  /**
   ...
   */
  applicabilityFor({ definedFeatures: features, supportedFeatures }) {
...
((Conformance2) => {
  ...
  ((Applicability2) => {
    ...
    Applicability2[Applicability2["Optional"] = 1] = "Optional";
    Applicability2[Applicability2["Conditional"] = 2] = "Conditional";
    Applicability2[Applicability2["Mandatory"] = 3] = "Mandatory";
  ...
  function serializeAtomic(ast, otherOperator) {
    ...
    if (otherOperator !== void 0 && isHigherPrecedence(otherOperator, ast.type)) {
      return `(${serialized})`;
    }
    return serialized;
  }
    ...
    switch (ast.type) {
      ...
      case "<=" /* LTE */:
        ...
        if (ast.param.lhs.type === "name") {
          ...
          if (referenced?.effectiveMetatype === import_common.Metatype.enum) {
  ...
  function serialize(ast) {
    switch (ast.type) {
      ...
      case "name" /* Name */:
      case "value" /* Value */:
        return import_common.FieldValue.serialize(ast.param);
      default:
        return ast.type;
    }
  }
...
function ParsedAst(conformance, definition) {
  ...
  const tokens = (0, import_parser.TokenStream)(import_parser.Lexer.Basic.lex(definition, (code, message) => conformance.error(code, message)));
  const otherwise = [];
  while (!tokens.done) {
  ...
  function parseExpression() {
    ...
    Parser.BinaryOperatorPrecedence.forEach((operators2) => {
      for (let i = 0; i < elements.length; i++) {
        if (operators2.indexOf(elements[i + 1]) !== -1) {
...
const operators = new Set(Object.values(Conformance.Operator));
function computeApplicability(features, supportedFeatures, conformance) {
  const { None, Optional, Conditional, Mandatory } = Conformance.Applicability;
  const { ast } = conformance;
  if (ast.type === "otherwise" /* Otherwise */) {
    let fallback = None;
  ...
  function assessOuterExpression(ast2) {
    switch (ast2.type) {
      case "choice" /* Choice */:
        return assessOuterExpression(ast2.param.expr);
      case "O" /* Optional */:
      case "empty" /* Empty */:
        return Optional;
      case "optionalIf" /* OptionalIf */:
        const applicability = assessInnerExpression(ast2.param);
    ...
    return assessInnerExpression(ast2);
  }
  function assessInnerExpression(ast2) {
    switch (ast2.type) {
      case "name" /* Name */:
        if (features.has(ast2.param)) {
          return supportedFeatures.has(ast2.param) ? Mandatory : None;
        }
        return Conditional;
      case "!" /* NOT */:
        switch (assessInnerExpression(ast2.param)) {
          case None:
            return Mandatory;
          case Conditional:
            return Conditional;
        ...
        break;
      case "&" /* AND */: {
        const lhs = assessInnerExpression(ast2.param.lhs);
        const rhs = assessInnerExpression(ast2.param.rhs);
      ...
      default:
        if (operators.has(ast2.type)) {
          return Optional;
        }
    }
    throw new import_general.InternalError(`Conformance ${conformance}: Invalid node type ${ast2.type} in inner expression`);
  }
}
