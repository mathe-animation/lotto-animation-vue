 ...
 */
const logger = import_general.Logger.get("ClusterHelper");
...
function getClusterNameById(clusterId) {
  return import_ClusterRegistry.ClusterRegistry.get(clusterId)?.name ?? `Unknown cluster ${import_general.Diagnostic.hex(clusterId)}`;
}
function getClusterById(clusterId) {
  let cluster = import_ClusterRegistry.ClusterRegistry.get(clusterId);
  if (cluster === void 0) {
    logger.info(`Unknown cluster ${toHex(clusterId)} requested: UnknownCluster instance added.`);
    cluster = UnknownCluster(clusterId);
    import_ClusterRegistry.ClusterRegistry.register(cluster);
  }
  return cluster;
}
