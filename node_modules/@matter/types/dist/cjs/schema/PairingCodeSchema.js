...
__export(PairingCodeSchema_exports, {
  ...
  MATTER_QR_CODE_ALL_PAYLOAD_MAX_LENGTH: () => MATTER_QR_CODE_ALL_PAYLOAD_MAX_LENGTH,
  MATTER_QR_CODE_SINGLE_PAYLOAD_MAX_LENGTH: () => MATTER_QR_CODE_SINGLE_PAYLOAD_MAX_LENGTH,
 ...
 */
const MATTER_QR_CODE_SINGLE_PAYLOAD_MAX_LENGTH = 255;
const MATTER_QR_CODE_ALL_PAYLOAD_MAX_LENGTH = 4296;
...
class QrPairingCodeSchema extends import_Schema.Schema {
  encodeInternal(payloadData) {
    ...
    const result = PREFIX + payloadData.map((payloadData2) => {
      const { tlvData } = payloadData2;
      const data = tlvData !== void 0 && tlvData.byteLength > 0 ? import_general.Bytes.of(import_general.Bytes.concat(QrCodeDataSchema.encode(payloadData2), tlvData)) : QrCodeDataSchema.encode(payloadData2);
      const result2 = import_Base38Schema.Base38.encode(data);
      if (result2.length > MATTER_QR_CODE_SINGLE_PAYLOAD_MAX_LENGTH) {
        throw new import_general.UnexpectedDataError(
          `Encoded pairing code is too long: ${result2.length} characters (max ${MATTER_QR_CODE_SINGLE_PAYLOAD_MAX_LENGTH})`
        );
      }
      return result2;
    }).join("*");
  ...
  decodeInternal(encoded) {
    ...
    return encoded.slice(PREFIX.length).split("*").map((encodedData) => {
      const data = import_general.Bytes.of(import_Base38Schema.Base38.decode(encodedData));
      return {
