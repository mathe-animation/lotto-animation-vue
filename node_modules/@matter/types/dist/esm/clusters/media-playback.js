...
((MediaPlayback2) => {
  ...
  class InvalidStateForCommandError extends StatusResponseError {
    constructor(message = "Requested playback command is invalid in the current playback state", code = GlobalStatus.Failure, clusterCode = 1 /* InvalidStateForCommand */) {
  ...
  class NotAllowedError extends StatusResponseError {
    constructor(message = "Requested playback command is not allowed in the current playback state. For example, attempting to fast-forward during a commercial might return NotAllowed", code = GlobalStatus.Failure, clusterCode = 2 /* NotAllowed */) {
      super(message, code, clusterCode);
    }
  }
  MediaPlayback2.NotAllowedError = NotAllowedError;
  class NotActiveError extends StatusResponseError {
    constructor(message = "This endpoint is not active for playback", code = GlobalStatus.Failure, clusterCode = 3 /* NotActive */) {
      super(message, code, clusterCode);
    }
  }
  MediaPlayback2.NotActiveError = NotActiveError;
  class SpeedOutOfRangeError extends StatusResponseError {
    constructor(message = "The FastForward or Rewind Command was issued but the media is already playing back at the fastest speed supported by the server in the respective direction", code = GlobalStatus.Failure, clusterCode = 4 /* SpeedOutOfRange */) {
      super(message, code, clusterCode);
    }
  }
  MediaPlayback2.SpeedOutOfRangeError = SpeedOutOfRangeError;
  class SeekOutOfRangeError extends StatusResponseError {
    constructor(message = "The Seek Command was issued with a value of position outside of the allowed seek range of the media", code = GlobalStatus.Failure, clusterCode = 5 /* SeekOutOfRange */) {
      super(message, code, clusterCode);
    }
  }
  ...
  MediaPlayback2.Base = MutableCluster.Component({
    ...
    events: {
       ...
       */
      stateChanged: OptionalEvent(0, Priority.Info, MediaPlayback2.TlvStateChangedEvent)
    },
    /**
