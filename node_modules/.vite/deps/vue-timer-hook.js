import { i as __toCommonJS, n as __esmMin, r as __exportAll, t as __commonJSMin } from "./chunk-Bu3CeSBz.js";
import { Ar as slotFlagsText, Cr as isVoidTag, Mr as toHandlerKey, Or as parseStringStyle, Sr as isSymbol, _r as isObject, ar as NO, br as isSVGTag, cr as camelize, dr as generateCodeFrame, fr as init_shared_esm_bundler, gr as isMathMLTag, hr as isHTMLTag, ir as EMPTY_OBJ, kr as shared_esm_bundler_exports, lr as capitalize, m as runtime_dom_esm_bundler_exports, mr as isBuiltInDirective, or as NOOP, pr as isArray, sr as PatchFlagNames, u as init_runtime_dom_esm_bundler, ur as extend, vr as isOn, wr as makeMap, xr as isString, yr as isReservedProp } from "./runtime-dom.esm-bundler-CNp80OcK.js";
...
//#endregion
//#region node_modules/vue-timer-hook/dist/vue-timer-hook.esm-bundler.js
/*!
* vue-timer-hook v1.0.84
* (c) 2025 Eduardo San Martin Morote
* @license MIT
*/
var require_vue_timer_hook_esm_bundler = /* @__PURE__ */ __commonJSMin(((exports) => {
	var vue = require_vue();
	...
	function useInterval(callback, ms) {
		...
		const start = (_ms) => {
			remove();
			if (!_ms && !ms) return;
			return intervalId = setInterval(callback, _ms || ms);
		};
	...
	var Time = class Time {
		static getTimeFromSeconds(secs) {
			...
			const days = vue.computed(() => Math.floor(totalSeconds.value / (3600 * 24)));
			const hours = vue.computed(() => Math.floor(totalSeconds.value % (3600 * 24) / 3600));
			const minutes = vue.computed(() => Math.floor(totalSeconds.value % 3600 / 60));
			return {
				seconds: vue.computed(() => Math.floor(totalSeconds.value % 60)),
				minutes,
				hours,
				days
			};
		...
		static getSecondsFromExpiry(expiry, shouldRound) {
			const milliSecondsDistance = expiry - (/* @__PURE__ */ new Date()).getTime();
		...
		static getSecondsFromPrevTime(prevTime, shouldRound) {
			const milliSecondsDistance = (/* @__PURE__ */ new Date()).getTime() - prevTime;
			if (milliSecondsDistance > 0) {
				const val = milliSecondsDistance / 1e3;
				return shouldRound ? Math.round(val) : val;
			}
		...
		static getFormattedTimeFromSeconds(totalSeconds, format) {
			...
			return {
				...
				minutes,
				hours: vue.computed(() => format === "12-hour" ? hours.value % 12 : hours.value),
				ampm
			};
		}
	};
	var Validate = class {
		static expiryTimestamp(expiryTimestamp) {
			...
			if (!isValid) console.warn("vue-timer-hook: { useTimer } Invalid expiryTimestamp settings", expiryTimestamp);
			return isValid;
		}
	};
	...
	function useStopwatch(offsetTimestamp = 60, autoStart = true) {
		...
		if (typeof offsetTimestamp === "object" && "value" in offsetTimestamp) vue.watch(offsetTimestamp, (newOffset) => {
	...
	function useTime(format = "24-hour") {
		...
		if (typeof format === "object" && "value" in format) vue.watch(format, (newFormat) => {
			formatValue.value = newFormat;
		});
		return { ...Time.getFormattedTimeFromSeconds(seconds, formatValue.value) };
	}
	...
	function useTimer(expiry = 60, autoStart = true) {
		...
		function resume() {
			...
			restart(time.setMilliseconds(time.getMilliseconds() + state.seconds * 1e3));
		}
		function start() {
			if (state.didStart) {
...
//#endregion
export default require_vue_timer_hook_esm_bundler();
